...
function move(startPos, endPos) {
  ...
  switch (endCell) {
    case EMPTY:
      terrain[endPos.row][endPos.col] = isPlayer ? PLAYER : BOX; // si la case darriver est empty on la remplacer par le joueur ou la boxe selon is players
      break;

    case GOAL:
      terrain[endPos.row][endPos.col] = isPlayer ? PLAYERONGOAL : BOXONGOAL; // si la case darriver est empty on la remplacer par boxongoal ou playerongoal
      break;
  }
}
